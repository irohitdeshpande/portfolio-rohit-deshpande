...
// Your portfolio information - customize this with your actual details!
...
class PortfolioRAGGroqPopulator {
  private documents: Array<{content: string, metadata: any}> = [];
  private ragService: NodeRAGServiceGroq;

  constructor() {
    console.log('🚀 Starting Portfolio RAG Population (Using Groq)...');
...
${project.highlights.map(highlight => `• ${highlight}`).join('\n')}`,
  ...
  private async ingestDocuments() {
    ...
    if (!this.ragService.isAvailable()) {
      throw new Error('❌ RAG services not available. Check your Groq API key and Pinecone API key in .env file.');
    }
  ...
  async run() {
    try {
      console.log('🎯 Portfolio RAG Population Starting (Groq-powered)...\n');
...
// Run the populator
const populator = new PortfolioRAGGroqPopulator();
populator.run();
